...
Preact 框架（版本 1 ~ 8）将组件实例和组件实例对应的 DOM 元素相互关联，组件实例通过 `base` 字段引用它的 DOM 节点，而 DOM 节点通过 `_component` 引用它的组件实例，从而实现组件实例的持久化保存，而且它直接把旧的 DOM 和新的 VNode 进行对比，边对比边修改 旧DOM，使得 diff 过程更快。
